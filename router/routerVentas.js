...
router.post("/insertventa", async (req,res)=>{
    ...
    tblDocproductos.map((p)=>{
     ...
     SELECT 
            '${sucursal}'AS EMP_NIT, YEAR(getdate()) AS DOC_ANO, MONTH(GETDATE()) AS DOC_MES,	
            ...
            ${p.DESCUENTO} AS DESCUENTOFAC,	0 AS PORDESCUFAC,	${p.DESCUENTO} AS TOTALDESCUENTO,	
    ...
    //variables sin asignar
    let diascredito = 1;
    let por_descuento = (Number(totaldescuento)/Number(totalprecio))*100;
    let ivadocumento = 0;
