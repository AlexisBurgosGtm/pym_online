let apigen = {
    ...
    clientesAjenosVendedor: async(sucursal,filtro,idContenedor)=>{
        ...
        .then((response) => {
            ...
            data.map((rows)=>{                    
                        strdata = strdata + `
                    <tr class='card border-mostaza hand card-rounded' onclick="getMenuCliente('${rows.CODIGO}','${rows.NOMCLIE}','${rows.DIRCLIE}','${rows.TELEFONO}','${rows.LAT}','${rows.LONG}','${rows.NIT}','${rows.SALDO_VENCIDO || 0}','${rows.TIPOCLIE}');">
